...
// ============================================================================
// runOneEligibility — run the eligibility engine for a single NF and persist
// the result. Used by both the per-claim route and run-all bulk endpoint.
//
// Options:
//   force — when true, overwrites rows with manually_confirmed_at set.
//            Default: false (those rows are skipped and returned as "skipped").
// ============================================================================

export type RunOneStatus =
  | "processed"         // engine ran + result saved
  | "skipped_confirmed" // manually_confirmed_at is set and force=false
  | "not_medical"       // NF is not is_medical / is_reimbursable — skip
  | "not_found"         // no NF with this id
  | "no_active_policy"  // no active insurance policy exists
...
export async function runOneEligibility(
  nfId: string,
  opts: Opts = {}
  ...
  const { data: nfRow, error: nfErr } = await sb
    ...
    .eq("id", nfId)
    .maybeSingle();
  if (nfErr) return { nf_id: nfId, status: "error", error: nfErr.message };
  if (!nfRow) return { nf_id: nfId, status: "not_found" };
  if (!nfRow.is_medical && !nfRow.is_reimbursable) {
    return { nf_id: nfId, status: "not_medical" };
  }
  ...
  const { data: policies, error: polErr } = await sb
    ...
    .limit(1);
  if (polErr) return { nf_id: nfId, status: "error", error: polErr.message };
  const polRow = policies?.[0];
  if (!polRow) return { nf_id: nfId, status: "no_active_policy" };
  ...
  // ── 3. Coverage rules + terms + dependents ───────────────────────────────────
  const [{ data: rulesRows }, { data: termRows }, { data: depRows }] =
    await Promise.all([
      sb
  ...
  const rules: CoverageRule[] = (rulesRows ?? []).map((r) => ({
    ...
    annual_limit_amount:
      r.annual_limit_amount != null ? Number(r.annual_limit_amount) : null,
  ...
  // ── 4. Existing claim + manually_confirmed guard ─────────────────────────────
  ...
  if (ec?.manually_confirmed_at && !force) {
    return { nf_id: nfId, status: "skipped_confirmed" };
  }
  ...
  // ── 5. Prior claims for annual-limit math ────────────────────────────────────
  ...
  const prior_claims: PriorClaim[] = ((priorRows ?? []) as PriorRow[]).map((p) => {
    ...
    return {
      ...
      nf_emission_date: nfRel?.emission_date
        ? String(nfRel.emission_date).slice(0, 10)
        : null,
    };
  });
  ...
  let result;
  try {
    result = await determineEligibility(input);
  } catch (e) {
    return { nf_id: nfId, status: "error", error: (e as Error).message };
  }
  ...
  if (saveErr) {
    return { nf_id: nfId, status: "error", error: saveErr.message };
  }
